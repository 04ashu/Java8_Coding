...
public class Most {
	...
	public static void main(String[] args) {
		...
		System.out.println("Print the duplicates");
		...
		int num = list.stream().findFirst().get();
		...
		System.out.println("Second Largest of the list");
		int secMax = list.stream().sorted(Collections.reverseOrder()).skip(1).findFirst().get();
		System.out.println(secMax);
		
		//to find duplicate elements from the list
		System.out.println("Duplicate Elements");
		list.stream().filter(n->Collections.frequency(list, n)>1).collect(Collectors.toSet()).forEach(System.out::println);
		Set setDup = new HashSet();
		list.stream().filter(n-> !setDup.add(n)).collect(Collectors.toSet()).forEach(System.out::println);
		...
		Map<Integer,Long> elementCount = list.stream().collect(Collectors.groupingBy(Function.identity(),Collectors.counting()));
		...
		int sumDigits = Stream.of(String.valueOf(51435).split("")).collect(Collectors.summingInt(Integer::parseInt));
