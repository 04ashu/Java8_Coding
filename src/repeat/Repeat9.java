...
public class Repeat9 {
	...
	public static void main(String[] args) {
		...
		String collectString = stringList.stream().collect(Collectors.joining(",", "{", "}"));
		...
		Map<Character, Long> countCharacter = str.chars().mapToObj(ch->(char)ch).collect(Collectors.groupingBy(Function.identity(),Collectors.counting()));
		...
		Integer sumofDigits = Stream.of(String.valueOf(12345).split("")).collect(Collectors.summingInt(Integer::parseInt));
		...
		boolean anyMatch = string1.stream().anyMatch(s->s.equals("Java"));
		System.out.println(anyMatch);
		
		System.out.println("Character of string occured more than twice");
		String str1 = "abababghgjgfhioem";
		Map<Character, Long> collect = str1.chars().mapToObj(ch->(char)ch).collect(Collectors.groupingBy(Function.identity(),Collectors.counting()));
		collect.entrySet().stream().filter(entry->entry.getValue()>2).map(Map.Entry::getKey).forEach(System.out::println);;
